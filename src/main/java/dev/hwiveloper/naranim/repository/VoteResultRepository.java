...
public interface VoteResultRepository extends CrudRepository<VoteResult, VoteResultPK>{
	@Query("SELECT DISTINCT v.key.sdName FROM woomin_vote_result v WHERE v.key.sgId=:sgId AND v.key.sgTypeCode=:sgTypeCode")
	public List<String> findDistinctSdNameByKey(String sgId, String sgTypeCode);
	
	@Query("SELECT DISTINCT v.key.sdName, v.key.sggName FROM woomin_vote_result v WHERE v.key.sgId=:sgId AND v.key.sgTypeCode=:sgTypeCode ORDER BY v.key.sdName, v.key.sggName, v.crOrder")
